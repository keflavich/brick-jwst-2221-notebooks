...
print('Loading abfix 10-band Vega catalog (F466N + 2221-band source)...')
ABFIX_CAT = Table.read(str(BRICK_ORANGE / 'catalogs' /
    'basic_merged_indivexp_photometry_tables_merged_ok2221or1182_20251211_abfix.fits'))
_abfix_coords = SkyCoord(ABFIX_CAT['skycoord_ref'])
_idx_afx, _sep_afx, _ = match_coordinates_sky(source_coords, _abfix_coords)
has_abfix = _sep_afx.arcsec < _XMATCH_RADIUS
print(f'  {has_abfix.sum()} / {len(targets)} matched to abfix')

def abfix_flux(i, filt):
    """Vega mag from the abfix catalog -> Jy (nan if unmatched/absent)."""
    if not has_abfix[i]:
        return np.nan
    col = f'mag_vega_{filt}'
    if col not in ABFIX_CAT.colnames:
        return np.nan
    m = _to_float(ABFIX_CAT[int(_idx_afx[i])][col])
    zp = NIRCAM_VEGA_ZP_JY.get(filt)
    if zp is None or not np.isfinite(m):
        return np.nan
...
# The target CSV's magnitude columns are AB in EVERY band (verified against the
# abfix Vega catalog 2026-07-19: offsets = AB-Vega exactly, MAD ~0.03). Convert
# before applying Vega zeropoints, or the SED gains per-band fake jumps.
CSV_AB_VEGA = {'f115w': 0.79, 'f182m': 1.58, 'f187n': 1.65, 'f200w': 1.69,
               'f212n': 1.83, 'f356w': 2.79, 'f405n': 3.12, 'f410m': 3.11,
               'f444w': 3.21, 'f466n': 3.45}


def vega_to_jy(mag, filt, from_csv=False):
    m = _to_float(mag)
    if from_csv and filt in CSV_AB_VEGA and np.isfinite(m):
        m = m - CSV_AB_VEGA[filt]
...
def get_source_fluxes(i):
    ...
    # NIRCam: satstar → m8_dedup → abfix(Vega, all 10 bands) → CSV → image
    for filt in NIRCAM_FILTERS:
        sf, sef = get_satstar_flux(filt, coord)
        if np.isfinite(sf):
            fluxes[filt] = (WAVELENGTHS_UM[filt], sf, sef, False); continue
        af = abfix_flux(i, filt)
        if np.isfinite(af):
            fluxes[filt] = (WAVELENGTHS_UM[filt], af, np.nan, False); continue
        if has_nircam[i]:
        ...
        if col in targets.columns:
            f = vega_to_jy(row[col], filt, from_csv=True)
            if np.isfinite(f):
